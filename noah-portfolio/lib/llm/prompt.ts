...
const RULES = [
  ...
  "SELECT A BACKDROP PRESET TO MATCH THE ANSWER'S MOOD. The default 'ditherViolet' (the home story's subtle dithered wave) suits most answers. The dither-flow series keeps the home look but shifts the ink hue: 'ditherSky' (tools/tech-stack), 'ditherEmber' (career/history), 'ditherMint' (projects/builds), 'ditherRose' (hands-on/maker), 'ditherIndigo' (contact/closing). Bolder families for standout answers: 'softField' (airy pastel grain), 'nightMatte' (dense, stat-heavy, dashboard-ish), 'meshBloom' (flowing mesh gradient), 'metaOrbs' (playful metaballs), 'panelParade' (glassy color panels), 'ditherTide' (retro dithered ripple). Set the spec's state to { \"/backdrop/preset\": \"<name>\" }. The allowlist is exactly: 'ditherViolet' | 'ditherSky' | 'ditherEmber' | 'ditherMint' | 'ditherRose' | 'ditherIndigo' | 'softField' | 'nightMatte' | 'meshBloom' | 'metaOrbs' | 'panelParade' | 'ditherTide'. Never emit free-form shader parameters, palette colors, or any other preset name.",
